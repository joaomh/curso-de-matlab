...
Idx      =        % Find all freqs with large power
PSDclean =        % Zero out all others
FCoef    =        % Zero out small Fourier coeffs. in Y
ffilt    =        % Inverse FFT for filtered time signal

% Plots
figure(2), clf
subplot(3,1,1)
plot(time,NoiseSignal,'r','LineWidth',1)
hold on
plot(time,OrgSignal,'k','LineWidth',1.5)
legend('Noisy','Clean')
subplot(3,1,2)
plot(time,NoiseSignal,'k','LineWidth',1.5)
hold on
plot(time,ffilt,'b','LineWidth',1)
legend('No Filter','Filtered')
subplot(3,1,3)
plot(VecTime(L),PSD(L),'r','LineWidth',1.5)
hold on
plot(VecTime(L),PSDclean(L),'-b','LineWidth',1)
