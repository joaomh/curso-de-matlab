...
for i = 1:20
    ...
    A(i) = sum(f.* cos(pi *i*x/L))*dx; % Inner product
    B(i) = sum(f.* sin(pi *i*x/L))*dx;
    funcFS = funcFS + A(i)* cos(i* pi *x/L) + B(i)* sin(i* pi *x/L); % Fourier Series
...
for i = 1:100
    ...
    Ak = sum(f.* cos(2* pi *i*x/L))*dx*2/L;
    Bk = sum(f.* sin(2* pi *i*x/L))*dx*2/L;
    funcFS = funcFS + Ak* cos(2*i* pi *x/L) + Bk* sin(2*i* pi *x/L); % Fourier Series
...
for i = 1:Points2
    ...
    ComplexSineWave2 = exp( -1j*2*pi*(i-1)*FourTime );
    subplot(5,4,i)
...
for i = 1:nPoints3
    ...
    ComplexSineWave3 = exp( -1i*2*pi*(i-1)*FourierTime3 );
    SignalX(i) = sum( Signal.*ComplexSineWave3 );
...
% Amplitude
SignalAmp = abs(SignalX(1:length(Hz))/nPoints3);
