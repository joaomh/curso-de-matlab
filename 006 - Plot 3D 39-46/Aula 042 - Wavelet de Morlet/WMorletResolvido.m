...
for i = 1:n
    ...
    Morlet(i,:) = cos(2*pi * freq(i)*tempo) .* exp(-(tempo.^2) / (2*sigma^2));
end

% Plot 2D
figure(1), clf
plot(tempo,Morlet(1,:),'linew',2)
title('Wavelet de Morlet')
xlabel('Tempo (sec.)'), ylabel('Amplitude')

% Surf
figure(2), clf
surf(tempo,freq,Morlet)
shading interp
colormap jet
rotate3d on
axis square
...
figure(3), clf
surf(tempo,freq,Morlet,repmat(freq',1,size(Morlet,2)))
shading interp
colormap jet
rotate3d on
axis square
