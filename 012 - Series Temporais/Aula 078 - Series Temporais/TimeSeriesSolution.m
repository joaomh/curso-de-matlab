...
kernelBack = zeros(2*k+1,1);
for i = 0:length(kernel)-1
     kernelBack(i+1) = kernel(2*k+1-i);
end
...
for i = 1+k:n-k
    ...
    % Part of the signal
    signalPart = signal(i-k:i+k);
    
    % Convolution at this point is the dot product
    convSize(i) = dot( kernelBack,signalPart );
end
...
convSize2 = ifft( sigX .* krnX );
convSize2 = convSize2(k+1:end-k); % cut off edge
